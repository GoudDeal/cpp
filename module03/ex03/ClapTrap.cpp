...
/*   Updated: 2022/08/19 00:25:34 by dcyprien         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ClapTrap::attack(const std::string & target){
	if (this->getAp() >= 1 && this->getHp() > 0)
	{
		...
		std::cout << this->getName() << " is dead ! Oh my god !" << std::endl;
	return ;
}
...
void	ClapTrap::beRepaired(unsigned int amount){
	if (this->getHp() != 0 && (this->getHp() + amount) > 100 && this->getAp() >= 1)
	{
		this->_hp = 100;
		std::cout << "Beep Boop! ClapTrap " << this->getName() << " tried to repair himself for " << amount << " HP but he can't have more than 100 HP !" << std::endl << this->getName() << " now has " << this->getHp() << " HP." << std::endl;
	}
	else if (this->getHp() <= 0)
		std::cout << "ClapTrap " << this->getName() << " is dead... Press F to pay your respects." << std::endl;
